// src/utils/database.js — BizTrack Pro v4.0
...
const SCHEMA = `
  CREATE TABLE IF NOT EXISTS settings (
    id INTEGER PRIMARY KEY, bizName TEXT DEFAULT 'My Business',
    owner TEXT DEFAULT '', type TEXT DEFAULT 'General Shop',
    currency TEXT DEFAULT 'UGX', payTerms INTEGER DEFAULT 30,
    taxRate REAL DEFAULT 0, lowStock INTEGER DEFAULT 5,
  ...
  CREATE TABLE IF NOT EXISTS inventory (
    ...
    -- Unit Conversion Engine fields
    baseUnit TEXT DEFAULT 'pcs',          -- what you sell in (e.g. kg, bottle, pc)
    purchaseUnit TEXT DEFAULT 'pcs',      -- what you buy in (e.g. bag, crate, box)
    conversionFactor REAL DEFAULT 1,      -- base units per purchase unit (e.g. 24 bottles/crate)
    -- Legacy alias kept for backward compat
    saleUnit TEXT DEFAULT 'pcs',
    unit TEXT DEFAULT 'pcs',
    -- WMA Costing fields
    wmaCost REAL DEFAULT 0,               -- current weighted moving average cost per BASE unit
    costPrice REAL DEFAULT 0,             -- alias for wmaCost (kept for compat)
    sellPrice REAL DEFAULT 0,
    stock REAL DEFAULT 0,                 -- always in BASE units
    reorderLevel REAL DEFAULT 5,          -- in BASE units; per-product, not overridden by settings
    supplierId TEXT,
    notes TEXT,
  ...
  CREATE TABLE IF NOT EXISTS wma_history (
    ...
    purchaseQty REAL NOT NULL,            -- qty in purchase units (e.g. 2 crates)
    purchaseUnit TEXT NOT NULL,
    baseUnitsAdded REAL NOT NULL,         -- purchaseQty × conversionFactor
    baseUnit TEXT NOT NULL,
    bulkCostPerPurchaseUnit REAL NOT NULL, -- invoice price per purchase unit
    newCostPerBaseUnit REAL NOT NULL,     -- bulkCostPerPurchaseUnit / conversionFactor
    -- WMA before/after
    ...
    -- Alert
    alertType TEXT NOT NULL,             -- INITIAL | NO_CHANGE | MARGIN_GAIN | PRICE_PROTECTION
    alertData TEXT,                      -- JSON blob with full alert details
    date TEXT DEFAULT (datetime('now'))
  );

  CREATE TABLE IF NOT EXISTS sales (
    id TEXT PRIMARY KEY, product TEXT NOT NULL, category TEXT,
    saleUnit TEXT DEFAULT 'pcs',
    qty REAL DEFAULT 1, unitPrice REAL DEFAULT 0, costPrice REAL DEFAULT 0,
    discount REAL DEFAULT 0, total REAL DEFAULT 0, paid REAL DEFAULT 0,
    balance REAL DEFAULT 0, status TEXT DEFAULT 'UNPAID',
    customer TEXT DEFAULT 'Walk-in', phone TEXT, method TEXT DEFAULT 'Cash',
    notes TEXT, dueDate TEXT, date TEXT DEFAULT (datetime('now'))
  );

  CREATE TABLE IF NOT EXISTS expenses (
    id TEXT PRIMARY KEY, category TEXT NOT NULL, description TEXT NOT NULL,
    amount REAL DEFAULT 0, method TEXT DEFAULT 'Cash',
    reference TEXT, date TEXT DEFAULT (datetime('now'))
  );

  CREATE TABLE IF NOT EXISTS payables (
    id TEXT PRIMARY KEY, creditor TEXT NOT NULL, category TEXT DEFAULT 'Supplier Invoice',
    description TEXT NOT NULL, amount REAL DEFAULT 0, amountPaid REAL DEFAULT 0,
    balance REAL DEFAULT 0, status TEXT DEFAULT 'UNPAID',
    dueDate TEXT, date TEXT DEFAULT (datetime('now')), notes TEXT
  );

  CREATE TABLE IF NOT EXISTS suppliers (
    id TEXT PRIMARY KEY, name TEXT NOT NULL, contact TEXT, phone TEXT,
    email TEXT, address TEXT, notes TEXT, createdAt TEXT DEFAULT (datetime('now'))
  );

  CREATE TABLE IF NOT EXISTS customers (
    id TEXT PRIMARY KEY, name TEXT NOT NULL, phone TEXT, email TEXT,
    address TEXT, notes TEXT, createdAt TEXT DEFAULT (datetime('now'))
  );

  CREATE TABLE IF NOT EXISTS returns_log (
    id TEXT PRIMARY KEY, saleId TEXT, product TEXT,
    qty REAL DEFAULT 1, refund REAL DEFAULT 0,
    reason TEXT, date TEXT DEFAULT (datetime('now'))
  );

  CREATE TABLE IF NOT EXISTS audit_log (
    id TEXT PRIMARY KEY, action TEXT NOT NULL,
    tableName TEXT NOT NULL, recordId TEXT,
    data TEXT, reason TEXT, date TEXT DEFAULT (datetime('now'))
  );
...
export async function initDB() {
  try {
    ...
    if (Capacitor.isNativePlatform()) {
      ...
      if (ret.result && isConn) db = await sqliteConnection.retrieveConnection(DB_NAME, false);
      else db = await sqliteConnection.createConnection(DB_NAME, false, 'no-encryption', DB_VERSION, false);
    } else {
      db = await sqliteConnection.createConnection(DB_NAME, false, 'no-encryption', DB_VERSION, false);
    }
    await db.open();

    // Run full schema (CREATE TABLE IF NOT EXISTS is idempotent)
    const stmts = SCHEMA.split(';').map(s => s.trim()).filter(s => s.length > 0);
    for (const s of stmts) await db.execute(s + ';');

    // Safe migrations for existing installs (ALTER TABLE IF NOT EXISTS column doesn't exist in SQLite,
    // so we wrap each in try/catch — harmless on fresh installs)
    const migrations = [
      // v3 → v4 payables table
    ...
    for (const m of migrations) { try { await db.execute(m + ';'); } catch(e) { /* column/table already exists */ } }

    // Backfill wmaCost from costPrice for existing rows
    await db.execute(`UPDATE inventory SET wmaCost = costPrice WHERE wmaCost = 0 AND costPrice > 0;`);
    await db.execute(`UPDATE inventory SET baseUnit = COALESCE(NULLIF(saleUnit,''), 'pcs') WHERE baseUnit = 'pcs' AND saleUnit != 'pcs';`);

    console.log('BizTrack DB v4.0 (WMA + UCE) ready');
    return true;
...
export async function addProduct(p) {
  ...
  await dbRun(
    ...
    [id, p.name, p.category || 'Other',
     ...
     p.sellPrice || 0, p.stock || 0,
     p.reorderLevel ?? 5,
     p.supplierId || '', p.notes || '']
  );

  // Record as initial WMA history entry if has opening stock
...
//   existingValue     = prevStock × prevWMACost
//   newValue          = baseUnitsAdded × newCostPerBase
//   newWMACost        = (existingValue + newValue) / (prevStock + baseUnitsAdded)
//
// Alert rules (price-direction neutral):
//   newWMACost < prevCost  → MARGIN_GAIN    (profit per unit increased)
//   newWMACost > prevCost  → PRICE_PROTECTION (suggest min sell to keep 30% margin)
//   no change / initial    → NO_CHANGE / INITIAL
//
export async function restockWithWMA(productId, purchaseQty, bulkCostPerPurchaseUnit, newSellPrice = null) {
  ...
  const currentSell  = newSellPrice ?? p.sellPrice ?? 0;

  // ── Weighted Moving Average ──────────────────────────────────────────────────
  let newWMACost;
  if (prevStock <= 0 || prevWMACost <= 0) {
    newWMACost = newCostPerBase;                          // No prior stock — new cost is the WMA
  } else {
    const existingValue = prevStock * prevWMACost;
    const newValue      = baseUnitsAdded * newCostPerBase;
    newWMACost = (existingValue + newValue) / (prevStock + baseUnitsAdded);
  }
  ...
  if (isInitial) {
    ...
    // Cost went DOWN — margin improved
    ...
    const profitGainPerUnit = prevWMACost - newWMACost;
    alertData = {
      prevWMACost,
      newWMACost,
      costDelta,
      ...
      profitGainPerUnit: +profitGainPerUnit.toFixed(4),
    };
  } else if (costDelta > 0.001) {
    // Cost went UP — check 30% margin floor
...
// Legacy shim — kept so old call-sites don't break; routes through WMA engine
export async function updateProductStock(id, newStock, costPrice, sellPrice) {
  const updates = ['stock=?', 'wmaCost=COALESCE(wmaCost,0)']; const vals = [newStock];
...
export async function addSale(s) {
  ...
  await dbRun(
    `INSERT INTO sales (id,product,category,saleUnit,qty,unitPrice,costPrice,discount,total,paid,balance,status,customer,phone,method,notes,dueDate,date) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
    [id, s.product, s.category || '', s.saleUnit || 'pcs', s.qty, s.unitPrice, s.costPrice || 0,
     s.discount || 0, s.total, s.paid || 0, s.balance ?? s.total, s.status || 'UNPAID',
...
export async function addPayable(p) {
  ...
  await dbRun(
    ...
    [id, p.creditor, p.category || 'Supplier Invoice', p.description, p.amount, p.amountPaid || 0,
     p.balance ?? p.amount, p.status || 'UNPAID', p.dueDate || '', p.date || new Date().toISOString(), p.notes || '']
  );
  return id;
}
