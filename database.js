// src/utils/database.js — BizTrack Pro SQLite Service
...
const SCHEMA = `
  ...
  CREATE TABLE IF NOT EXISTS inventory (
    ...
    unit TEXT DEFAULT 'pcs', costPrice REAL DEFAULT 0, sellPrice REAL DEFAULT 0,
    stock INTEGER DEFAULT 0, reorderLevel INTEGER DEFAULT 5,
    supplierId TEXT, notes TEXT, createdAt TEXT DEFAULT (datetime('now'))
  );
...
export async function initDB() {
  try {
    ...
    // Migration: add payables table to existing installs
    try { await db.execute(`CREATE TABLE IF NOT EXISTS payables (id TEXT PRIMARY KEY, creditor TEXT NOT NULL, category TEXT DEFAULT 'Supplier Invoice', description TEXT NOT NULL, amount REAL DEFAULT 0, amountPaid REAL DEFAULT 0, balance REAL DEFAULT 0, status TEXT DEFAULT 'UNPAID', dueDate TEXT, date TEXT DEFAULT (datetime('now')), notes TEXT);`); } catch(e) {}
    console.log('BizTrack DB ready');
    return true;
  } catch (err) {
...
export async function saveSettings(s) {
  await dbRun(`UPDATE settings SET bizName=?,owner=?,type=?,currency=?,payTerms=?,taxRate=?,lowStock=?,invoiceFooter=? WHERE id=1`,
...
export async function addProduct(p) {
  ...
  await dbRun(`INSERT INTO inventory (id,name,category,unit,costPrice,sellPrice,stock,reorderLevel,supplierId,notes) VALUES (?,?,?,?,?,?,?,?,?,?)`,
    [id,p.name,p.category,p.unit,p.costPrice,p.sellPrice,p.stock,p.reorderLevel,p.supplierId||'',p.notes||'']);
  return id;
}
export async function updateProductStock(id, newStock, costPrice, sellPrice) {
  const updates = ['stock=?']; const vals = [newStock];
  if (costPrice !== null && costPrice !== undefined) { updates.push('costPrice=?'); vals.push(costPrice); }
  if (sellPrice !== null && sellPrice !== undefined) { updates.push('sellPrice=?'); vals.push(sellPrice); }
  vals.push(id);
  await dbRun(`UPDATE inventory SET ${updates.join(',')} WHERE id=?`, vals);
}
export async function deleteProduct(id) { await dbRun('DELETE FROM inventory WHERE id=?',[id]); }
...
export async function addSale(s) {
  ...
  await dbRun(`INSERT INTO sales (id,product,category,qty,unitPrice,costPrice,discount,total,paid,balance,status,customer,phone,method,notes,dueDate,date) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
    [id,s.product,s.category||'',s.qty,s.unitPrice,s.costPrice||0,s.discount||0,s.total,s.paid||0,s.balance||s.total,s.status||'UNPAID',s.customer||'Walk-in',s.phone||'',s.method||'Cash',s.notes||'',s.dueDate||'',s.date||new Date().toISOString()]);
...
export async function recordPayment(saleId, amount) {
  const sale = await getSaleById(saleId); if (!sale) return;
...
export async function deleteSale(id) { await dbRun('DELETE FROM sales WHERE id=?',[id]); }
...
export async function deleteExpense(id) { await dbRun('DELETE FROM expenses WHERE id=?',[id]); }
...
export async function addPayable(p) {
  ...
  await dbRun(`INSERT INTO payables (id,creditor,category,description,amount,amountPaid,balance,status,dueDate,date,notes) VALUES (?,?,?,?,?,?,?,?,?,?,?)`,
    [id,p.creditor,p.category||'Supplier Invoice',p.description,p.amount,p.amountPaid||0,p.balance||p.amount,p.status||'UNPAID',p.dueDate||'',p.date||new Date().toISOString(),p.notes||'']);
  return id;
}
export async function settlePayable(payableId, amount) {
  const rows = await dbQuery('SELECT * FROM payables WHERE id=?',[payableId]);
  const p = rows[0]; if (!p) return;
  const newPaid = (p.amountPaid||0) + amount;
  const newBalance = Math.max(0,(p.amount||0)-newPaid);
  const status = newBalance <= 0 ? 'PAID' : 'PARTIAL';
  await dbRun('UPDATE payables SET amountPaid=?,balance=?,status=? WHERE id=?',[newPaid,newBalance,status,payableId]);
  return { amountPaid: newPaid, balance: newBalance, status };
}
export async function deletePayable(id) { await dbRun('DELETE FROM payables WHERE id=?',[id]); }
...
export async function deleteSupplier(id) { await dbRun('DELETE FROM suppliers WHERE id=?',[id]); }
...
export async function getReportData(fromDate, toDate) {
  ...
  return {
    sales:    await dbQuery(`SELECT * FROM sales WHERE date>=? AND date<=? ORDER BY date DESC`,[fromDate,end]),
    expenses: await dbQuery(`SELECT * FROM expenses WHERE date>=? AND date<=? ORDER BY date DESC`,[fromDate,end]),
    returns:  await dbQuery(`SELECT * FROM returns_log WHERE date>=? AND date<=? ORDER BY date DESC`,[fromDate,end]),
    payables: await dbQuery(`SELECT * FROM payables WHERE date>=? AND date<=? ORDER BY date DESC`,[fromDate,end]),
  };
}
...
export async function importAllData(data) {
  for (const t of ['sales','inventory','expenses','payables','suppliers','customers','returns_log'])
